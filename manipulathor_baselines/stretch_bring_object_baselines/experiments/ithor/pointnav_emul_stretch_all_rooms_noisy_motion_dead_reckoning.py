...
from utils.stretch_utils.stretch_thor_sensors import RGBSensorStretchIntel, DepthSensorStretchIntel, \
    ...
    RGBSensorStretchKinectZero, AgentBodyPointNavEmulSensorDeadReckoning, ArmPointNavEmulSensorDeadReckoning, \
...
class PointNavEmulStretchAllRooms(
    ...
    SENSORS = [
        ...
        AgentBodyPointNavEmulSensorDeadReckoning(type='source', mask_sensor=source_mask_sensor_intel, depth_sensor=depth_sensor_intel),
        AgentBodyPointNavEmulSensorDeadReckoning(type='destination', mask_sensor=destination_mask_sensor_intel, depth_sensor=depth_sensor_intel),
        ArmPointNavEmulSensorDeadReckoning(type='source', mask_sensor=source_mask_sensor_kinect, depth_sensor=depth_sensor_kinect),
        ArmPointNavEmulSensorDeadReckoning(type='destination', mask_sensor=destination_mask_sensor_kinect, depth_sensor=depth_sensor_kinect),
    ...
    def test_task_sampler_args(self, **kwargs):
        ...
        if platform.system() == "Darwin":
            ...
            for sensor_type in sampler_args['sensors']:
                if isinstance(sensor_type, AgentBodyPointNavEmulSensorDeadReckoning):
                    sensor_type.device = torch.device(kwargs["devices"][0])

            for sensor_type in sampler_args['sensors']:
                if isinstance(sensor_type, ArmPointNavEmulSensorDeadReckoning):
                    sensor_type.device = torch.device(kwargs["devices"][0])
