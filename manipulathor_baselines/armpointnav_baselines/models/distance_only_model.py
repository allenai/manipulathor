...
class DistanceOnlyModel(ActorCriticModel[CategoricalDistr]):
    ...
    def __init__(
        ...
        distance_dist_embedding_size = torch.Tensor([1, 100, obj_state_embedding_size])
        self.distance_dist_embedding = input_embedding_net(
            distance_dist_embedding_size.long().tolist(), dropout=0
        )
    ...
    def get_distance_distance_embedding(
            self, state_tensor: torch.Tensor
    ) -> torch.FloatTensor:

        return self.distance_dist_embedding(state_tensor)
    ...
    def forward(  # type:ignore
        ...
        arm2obj_dist = self.get_distance_distance_embedding(
            observations["distance_agent_arm_to_obj"]
        )
        obj2goal_dist = self.get_distance_distance_embedding(
            observations["distance_obj_to_goal"]
        )
