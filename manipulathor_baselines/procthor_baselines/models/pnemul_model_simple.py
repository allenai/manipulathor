...
class PointNavEmulModelSimple(ActorCriticModel[CategoricalDistr]):
    ...
    def forward(  # type:ignore
        ...
        source_predicted_mask = observations['object_mask_source']
        destination_predicted_mask = observations['object_mask_destination']
        object_target_masks = source_predicted_mask.clone()
        object_target_masks[after_pickup] = destination_predicted_mask[after_pickup].clone()


        visual_observation = torch.cat([observations['depth_lowres'], observations['rgb_lowres'], object_target_masks], dim=-1).float()
        ...
        if self.visualize:
            ...
            hacky_visualization(observations, object_mask=object_target_masks, base_directory_to_right_images=self.starting_time, gt_mask=gt_mask)
