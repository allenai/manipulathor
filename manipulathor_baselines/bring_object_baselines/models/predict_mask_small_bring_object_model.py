        ...
        if pickup_bool.shape != torch.zeros((1,1)).shape:
            ...
            for i in range(b_size):
                for j in range(seq_len):
                    self.log_all(pickup_bool[i:i+1, j:j+1], gt_masks[i:i+1, j:j+1], predicted_mask[i:i+1, j:j+1], category_id[i:i+1, j:j+1], all_masks[i:i+1, j:j+1])
        else:
            self.log_all(pickup_bool, gt_masks, predicted_mask, category_id, all_masks)

    def log_all(self, pickup_bool, gt_masks, predicted_mask, category_id, all_masks):
