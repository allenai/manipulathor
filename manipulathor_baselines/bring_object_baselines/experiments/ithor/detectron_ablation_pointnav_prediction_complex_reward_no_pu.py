...
from ithor_arm.near_deadline_sensors import PointNavEmulatorSensor, PredictionObjectMask, DetectronPredictionObjectMask
...
class DetectronPredictionPointNavNoPUNewModelAndHand(
    ...
    source_mask_sensor_prediction = DetectronPredictionObjectMask(type='source',object_query_sensor=category_sample_source, rgb_for_detection_sensor=rgb_for_detection_sensor)
    destination_mask_sensor_prediction = DetectronPredictionObjectMask(type='destination',object_query_sensor=category_sample_destination, rgb_for_detection_sensor=rgb_for_detection_sensor)
    ...
    def test_task_sampler_args(self, **kwargs):
        ...
        if platform.system() == "Darwin":
            ...
            for sensor_type in sampler_args['sensors']:
                ...
                if isinstance(sensor_type, DetectronPredictionObjectMask):
                    sensor_type.device = torch.device(kwargs["devices"][0])
        return sampler_args
