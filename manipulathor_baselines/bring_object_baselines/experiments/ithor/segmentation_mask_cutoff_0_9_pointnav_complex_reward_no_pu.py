...
from ithor_arm.near_deadline_sensors import PointNavEmulatorSensor, MisDetectionNoisyObjectMask, MaskCutoffNoisyObjectMask
...
class PointNavNewModelAndHandMaskCutoffAblation(
    ...
    input."""
    NOISE_LEVEL = 0
    mask_cutoff_percent=0.9
    distance_thr = 1.5 # is this a good number?
    source_mask_sensor = MaskCutoffNoisyObjectMask(mask_cutoff_percent=mask_cutoff_percent, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='source', distance_thr=distance_thr)
    destination_mask_sensor = MaskCutoffNoisyObjectMask(mask_cutoff_percent=mask_cutoff_percent, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='destination', distance_thr=distance_thr)
    SENSORS = [
        RGBSensorThor(
    ...
    def train_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandMaskCutoffAblation, self).train_task_sampler_args(**kwargs)
    ...
    def test_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandMaskCutoffAblation, self).test_task_sampler_args(**kwargs)
        if platform.system() == "Darwin":
            pass
        else:
