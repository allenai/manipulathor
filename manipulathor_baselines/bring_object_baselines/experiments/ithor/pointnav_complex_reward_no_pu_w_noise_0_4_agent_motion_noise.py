...
class PointNavNewModelAndHandWAgentLocationAndMotionNoise(
    ...
    def train_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandWAgentLocationAndMotionNoise, self).train_task_sampler_args(**kwargs)
    ...
    def test_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandWAgentLocationAndMotionNoise, self).test_task_sampler_args(**kwargs)
        if platform.system() == "Darwin":
            pass
        else:
    ...
    def __init__(self):
        ...
        self.ENV_ARGS['motion_noise'] = dict()
        self.ENV_ARGS['motion_noise']['ahead_noise_meta_dist_params'] = {'bias_dist': [0,0.02], 'variance_dist': [0,0.05]}
        self.ENV_ARGS['motion_noise']['lateral_noise_meta_dist_params'] = {'bias_dist': [0,0.02], 'variance_dist': [0,0.02]}
        self.ENV_ARGS['motion_noise']['turning_noise_meta_dist_params'] = {'bias_dist': [0,5], 'variance_dist': [0,5]}
