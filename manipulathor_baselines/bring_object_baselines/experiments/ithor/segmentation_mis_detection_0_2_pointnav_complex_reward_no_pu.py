...
from ithor_arm.near_deadline_sensors import PointNavEmulatorSensor, MisDetectionNoisyObjectMask
...
class PointNavNewModelAndHandMisDetectionAblation(
    ...
    input."""
    NOISE_LEVEL = 0
    misdetection_percent=0.2
    distance_thr = 1.5 # is this a good number?
    source_mask_sensor = MisDetectionNoisyObjectMask(misdetection_percent=misdetection_percent, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='source', distance_thr=distance_thr)
    destination_mask_sensor = MisDetectionNoisyObjectMask(misdetection_percent=misdetection_percent, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='destination', distance_thr=distance_thr)
    SENSORS = [
        RGBSensorThor(
    ...
    def train_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandMisDetectionAblation, self).train_task_sampler_args(**kwargs)
    ...
    def test_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandMisDetectionAblation, self).test_task_sampler_args(**kwargs)
        if platform.system() == "Darwin":
            pass
        else:
