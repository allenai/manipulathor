...
class PointNavNewModelAndHandRecallAblation(
    ...
    source_mask_sensor = NoisyObjectMask(recall_percent=RECALL_PERCENT, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='source', distance_thr=distance_thr)
    destination_mask_sensor = NoisyObjectMask(recall_percent=RECALL_PERCENT, height=BringObjectiThorBaseConfig.SCREEN_SIZE, width=BringObjectiThorBaseConfig.SCREEN_SIZE,noise=NOISE_LEVEL, type='destination', distance_thr=distance_thr)
    SENSORS = [
        RGBSensorThor(
    ...
    def train_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandRecallAblation, self).train_task_sampler_args(**kwargs)
    ...
    def test_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavNewModelAndHandRecallAblation, self).test_task_sampler_args(**kwargs)
        if platform.system() == "Darwin":
            pass
        else:
