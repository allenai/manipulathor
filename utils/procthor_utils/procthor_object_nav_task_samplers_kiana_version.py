...
class ProcTHORObjectNavKianaVersionTaskSampler(ProcTHORObjectNavTaskSampler): #TODO too entangled
    ...
    def get_target_locations(self, forced_house_index=False, custom_index=0):
        ...
        object_types = list(set([o['objectType'] for o in valid_objects]))
        chosen_object_type = random.choice(object_types)
        target_object_ids = [o['objectId'] for o in valid_objects if o['objectType'] == chosen_object_type]
        room = random.choice(all_rooms)
        agent_initial_pose = random.choice(house_id_to_room_to_agent_pose[room])

        return dict(
            target_object_ids=target_object_ids,
            target_object_type=chosen_object_type,
            starting_pose=agent_initial_pose,
    ...
    def next_task(
        ...
        while data_point is None:
            ...
            data_point = self.get_target_locations()
        target_object_ids = data_point['target_object_ids']
        target_object_type = data_point['target_object_type']
        start_pose = data_point['starting_pose']
