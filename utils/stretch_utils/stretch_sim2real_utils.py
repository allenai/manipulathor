...
def kinect_reshape(frame):
    ...
    elif (len(frame.shape) == 3 and frame.shape[2] ==1) or len(frame.shape) == 2:
        frame = clip_depth_kinect_frame(frame)
...
remake_mask_prob = 0.1 #TODO are we happy with this approach"?
PIX_NOISE = 20 #TODO are we happy with this?
...
def clip_depth_kinect_frame(frame):
    ...
    if DEPTH_KINECT_MASK_FRAMES is None or DEPTH_KINECT_MASK_FRAMES.shape[0] != w or DEPTH_KINECT_MASK_FRAMES.shape[1] != h or random.random() < remake_mask_prob:
        DEPTH_KINECT_MASK_FRAMES = set_mask_kinect_depth(w, h)
    frame[(1 - DEPTH_KINECT_MASK_FRAMES).astype(bool)] = 0
    return frame

def clip_rgb_kinect_frame(frame):
    #TODO this does not cover the curve on top
