...
from ithor_arm.rearrangement_constants import (
    CONTROLLER_COMMIT_ID,
    MAX_HAND_METERS,
    OBJECT_TYPES_TO_NOT_MOVE
)
from ithor_arm.rearrange_utils import (
    BoundedFloat,
    ...
    def __init__(
        ...
        if ai2thor.__version__ is not None:  # Allows for custom THOR installs
            if ai2thor.__version__ not in ["0.0.1", None] and version.parse(
                ai2thor.__version__
            ) < version.parse(CONTROLLER_COMMIT_ID):
                raise ImportError(
                    f"To run the rearrangment baseline experiments you must use"
                    f" ai2thor version {CONTROLLER_COMMIT_ID} or higher."
                )
